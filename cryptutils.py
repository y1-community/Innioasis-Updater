...
# Lazy imports for crypto functionality
def _get_crypto_modules():
    """Lazy load crypto modules to avoid import errors"""
    try:
        from Crypto.Cipher import AES
        from Crypto.Util import Counter
        from Crypto.Hash import CMAC
        from Crypto.Util.number import long_to_bytes, bytes_to_long
        return AES, Counter, CMAC, long_to_bytes, bytes_to_long
    except ImportError as e:
        raise ImportError(f"Crypto modules not available. Please install pycryptodome: pip install pycryptodome. Original error: {e}")

def _lazy_crypto_import():
    """Get crypto modules with lazy loading"""
    global AES, Counter, CMAC, long_to_bytes, bytes_to_long
    if 'AES' not in globals():
        AES, Counter, CMAC, long_to_bytes, bytes_to_long = _get_crypto_modules()
    ...
    class Aes:
        ...
        class AesGcm:
            ...
            def change_key(self, master_key):
                _lazy_crypto_import()  # Load crypto modules when needed
