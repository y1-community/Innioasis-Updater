...
# Lazy import FUSE only when needed
FUSE = None
MtkDaFS = None
...
class DaHandler(metaclass=LogBase):
    ...
    def _load_fuse_if_needed(self):
        """Lazy load FUSE only when the fs command is used"""
        global FUSE, MtkDaFS
        if FUSE is None:
            try:
                ...
                self.error(f"FUSE library not available: {e}")
                self.error("Please install FUSE to use the filesystem mounting feature")
                return False
        return True
    ...
    def handle_da_cmds(self, mtk, cmd: str, args):
        ...
        if cmd == "gpt":
            ...
            if self._load_fuse_if_needed():
                ...
                self.error("Cannot mount filesystem: FUSE not available")
                return
        elif cmd == "footer":
            filename = args.filename
